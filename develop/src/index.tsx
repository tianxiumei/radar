...
export default class VisualizationStore extends VisualizationBase {
 ...
 getMetricDatas(dataset: IDataset, config: IKeyValues,precision:number){
  ...
  updateView(dataset: IDataset, config: IKeyValues) {
    ...
    dv.transform({
      type: 'fold',
      fields: type===Type.BUCKET? metrics&&metrics.length>0?metrics:[]:zipData[bucketFieldIndex]&&zipData[bucketFieldIndex].length>0?zipData[bucketFieldIndex].slice(0,maxFields?maxFields:10):[], // 展开字段集
      key: 'user', // key字段
      value: 'data', // value字段
    });
